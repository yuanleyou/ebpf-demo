...
{
	struct execsnoop_ppid_bpf *skel;
	...
	err = execsnoop_ppid_bpf__load(skel);
	if (err) {
	...
	err = execsnoop_ppid_bpf__attach(skel);
	if (err) {
 ...
 cleanup:
	...
	execsnoop_ppid_bpf__destroy(skel);
	return err != 0;
}
